class urlFotoUseCase {
  ...
  async urlFotoCreate(data) {
    ...
    const verifyUrlFoto = await this.urlFotoRepository.getFotoByUserId(
      data.userId
    );
    if (verifyUrlFoto !== null) {
      for (let i = 0; i < verifyUrlFoto.length; i++) {
        await this.urlFotoRepository.deleteUrlFoto(verifyUrlFoto[i].id);
      }
    }
